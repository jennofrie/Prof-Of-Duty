...
const QUALITY = String(args.q ?? 'ultra');
const PREWARM = String(args.prewarm ?? '1');
...
const angle = args.angle ?? (process.platform === 'darwin' ? 'metal' : null);
if (angle) browserArgs.unshift(`--use-angle=${angle}`);

const browser = await chromium.launch({
  headless: HEADLESS,
  args: browserArgs,
  executablePath: args.browser ? resolve(String(args.browser)) : undefined,
});
...
let failed = null;
try {
  await page.goto(`http://127.0.0.1:${PORT}/?capture=1&shot=${encodeURIComponent(SHOT)}&q=${encodeURIComponent(QUALITY)}&prewarm=${encodeURIComponent(PREWARM)}`, {
  ...
  if (args.list) {
    ...
    console.log(JSON.stringify({ ok: true, out: OUT, shot: SHOT, quality: QUALITY, prewarm: PREWARM, headless: HEADLESS, w: W, h: H, info }, null, 2));
  }
} catch (e) {
  failed = e;
} finally {
